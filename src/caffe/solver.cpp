...
typedef int num_iter;
typedef float iter_loss;
static vector<pair<num_iter, iter_loss>> loss_arr;
static std::mutex loss_mutex;
...
string fcCurrentTimeFileString()
{
...
template<typename Dtype>
void Solver<Dtype>::attachCCSolver(){
	this->ccSolver_ = new cc::Solver();
	this->ccSolver_->setNative(this);
}

template<typename Dtype>
Solver<Dtype>::~Solver(){
	if (this->ccSolver_){
		delete this->ccSolver_;
		this->ccSolver_ = 0;
	}
}
...
static void updateShow(void *){
	int num_init = 20;
	Mat frame = Mat::zeros(500, 850, CV_8UC3);
	int padding = 20;
	int padding_right = 50;
	Point ori(20, frame.rows - 20);

	line(frame, ori, Point(ori.x, padding), Scalar(0, 255), 2);
	line(frame, ori, Point(frame.cols - padding_right, ori.y), Scalar(0, 255), 2);

	vector<pair<num_iter, iter_loss>> local_loss_arr;
	while (g_keep_run){
		if (g_change_loss){
			g_change_loss = false;
			frame.setTo(0);
			...
			loss_mutex.lock();
			//local_loss_arr = loss_arr;
			if (loss_arr.size() > local_loss_arr.size())
				local_loss_arr.insert(local_loss_arr.end(), loss_arr.begin() + local_loss_arr.size(), loss_arr.end());
			...
			float maxloss = local_loss_arr.size()>0 ? std::max_element(local_loss_arr.begin(), local_loss_arr.end(), [](const pair<num_iter, iter_loss>& a, const pair<num_iter, iter_loss>& b){
				return a.second < b.second;
			})->second : 0;

			paDrawString(frame, format("loss: %.2f", maxloss), Point(ori.x, padding - 15), Scalar(0, 255), 12, false);
			maxloss = min(maxloss, local_loss_arr.size()>1000 ? 8 : maxloss);

			int width_plane = max(num_init, local_loss_arr.size());
			int width_real = frame.cols - padding - padding_right;
			int height_real = frame.rows - padding * 2;

			int num_aix = 8;
			int step_aix = width_plane / num_aix;
			float prev_rx = 0, prev_ry = 0;
			int max_num_point = min(local_loss_arr.size(), width_real / 2);
			int step_num_point = max_num_point == 0 ? 1 : local_loss_arr.size() / max_num_point;
			int aix_pos = 0;
			for (int i = 0; i < local_loss_arr.size(); i += step_num_point){
				float x = i / (float)width_plane * width_real;
				float y = local_loss_arr[i].second / maxloss * height_real;
				float rx = ori.x + x;
				float ry = ori.y - y;
				ry = max(ry, padding);

				if (i > 0){
					line(frame, Point(prev_rx, prev_ry), Point(rx, ry), Scalar(0, 0, 255), 1);
				}
				...
				if (i >= aix_pos){
					aix_pos += step_aix;
					paDrawString(frame, format("%d", local_loss_arr[i].first), Point(rx, ori.y + 3), Scalar(0, 255), 12, false);
				}
			}

			if (local_loss_arr.size()>0){
				int i = local_loss_arr.size() - 1;
				float x = i / (float)width_plane * width_real;
				float y = local_loss_arr[i].second / maxloss * height_real;
				float rx = ori.x + x;
				float ry = ori.y - y;
				ry = max(ry, padding);
				line(frame, Point(prev_rx, prev_ry), Point(rx, ry), Scalar(0, 0, 255), 1);
				paDrawString(frame, format("loss: %.2f", local_loss_arr[i].second), Point(rx - 30, ry - 20), Scalar(0, 255), 12, false);
			}
		}
		imshow("loss_map", frame);
		waitKey(1);
	}

	ReleaseSemaphore(g_exit_show_samp, 1, 0);
}
...
void exitShowThread(){
	...
	if (g_keep_run){
		g_keep_run = false;
		WaitForSingleObject(g_exit_show_samp, -1);
	}
}
...
void Solver<Dtype>::Init(const SolverParameter& param) {
  ...
  if (param_.show_realtime_loss()){
	  g_exit_show_samp = CreateSemaphoreA(0, 0, 1, 0);
	  g_keep_run = true;
	  _beginthread(updateShow, 0, 0);
  }
}
...
void Solver<Dtype>::Step(int iters) {
  ...
  while (iter_ < stop_iter) {
	...
	if (param_.save_loss() || param_.show_realtime_loss()){
		loss_mutex.lock();

		if (param_.show_realtime_loss()){
			loss_arr.push_back(make_pair(iter_, smoothed_loss_));
			g_change_loss = true;
		}
		loss_mutex.unlock();
	}
