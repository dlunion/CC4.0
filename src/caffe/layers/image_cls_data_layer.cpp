...
#include "caffe/layers/image_cls_data_layer.hpp"
...
ImageClsDataLayer<Dtype>::~ImageClsDataLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void ImageClsDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK(transform_param.has_mean_file() == false) << 
         "ImageClsDataLayer does not support mean file";
  ...
  while (std::getline(infile, linestr)) {
    ...
    lines_.push_back(std::make_pair(imgfn, clsfn));
  }
  ...
  //-------
  // label dimensions, for each image, stores a flag: 1 / 0 
  top[1]->Reshape(batch_size, 1, 1, 1);  
  this->transformed_label_.Reshape(batch_size, 1, 1, 1);
  for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
...
template <typename Dtype>
void ImageClsDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void ImageClsDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (label_type == ImageDataParameter_LabelType_IMAGE) {
      img_cls = atoi(lines_[lines_id_].second.c_str());
    } else {
      LOG(INFO) << "No label";
    }
    ...
    this->data_transformer_->TransformImgAndCls(cv_img, img_cls, 
...
INSTANTIATE_CLASS(ImageClsDataLayer);
REGISTER_LAYER_CLASS(ImageClsData);
