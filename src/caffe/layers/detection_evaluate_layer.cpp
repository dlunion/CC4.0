...
void DetectionEvaluateLayer<Dtype>::Forward_cpu(
  ...
  //获取检测到的结果，保存在all_detections里面，此时该类型的key是image_id
  //LabelBBox是一个map<int, vector<NormalizedBBox>>的结构，LabelBBox的key是class_id
  //即每个类别在该图片上检测出来的boundingbox的位置
  //det_data是7个float为一个object的方式编码存储信息
  ...
  //和all_detection类似，从gt_data（输入层提供的label数据）恢复出box信息存储到all_gt_bboxs
  // Retrieve all ground truth (including difficult ones).
  map<int, LabelBBox> all_gt_bboxes;
  GetGroundTruth(gt_data, bottom[1]->height(), background_label_id_,
                 true, &all_gt_bboxes);

  //获取输出数据指针，这个评估层的输出，是如下格式：
  //5个float组成：image_id，label，bbox.score，flag1，flag2
  //此时的flag1、flag2是用来区分true positive(flag1=1, flag2=0)，false positive(flag1=0, flag2=1)
