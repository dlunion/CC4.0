...
ctcStatus_t reduce_negate(const double *input, double *output, int rows, int cols, bool axis, cudaStream_t stream) {
	return reduce(ctc_helper::negate<double>(), ctc_helper::add<double>(), input, output, rows, cols, axis, stream);
}

ctcStatus_t reduce_exp(double const *input, double *output, int rows, int cols, bool axis, cudaStream_t stream) {
	return reduce(ctc_helper::exponential<double>(), ctc_helper::add<double>(), input, output, rows, cols, axis, stream);
}

ctcStatus_t reduce_max(double const *input, double *output, int rows, int cols, bool axis, cudaStream_t stream) {
	return reduce(ctc_helper::identity<double>(), ctc_helper::maximum<double>(), input, output, rows, cols, axis, stream);
}