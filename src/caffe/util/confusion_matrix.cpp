...
double ConfusionMatrix::maxFmeasure() const {
  double beta_square = 0.3;
  double precision = avgPrecision();
  double recall = avgRecall(false);
  double up = (1+beta_square) * precision * recall;
  double down = beta_square*precision + recall ;
  if (down == 0)
    down = 1;
