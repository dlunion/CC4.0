...
void ReadNetParamsFromTextFileOrDie(const string& param_file,
  ...
  //CHECK(ReadProtoFromTextFile(param_file, param))
   //   << "Failed to parse NetParameter file: " << param_file;
	bool isok = ReadProtoFromTextFile(param_file, param);
  if (!isok) throw std::exception("Failed to parse NetParameter");
  UpgradeNetAsNeeded(param_file, param);
}

void ReadNetParamsFromTextStringOrDie(const string& param_string,
	NetParameter* param) {
	//CHECK(ReadProtoFromTextString(param_string, param))
	//	<< "Failed to parse NetParameter file: " << "x";
	bool isok = ReadProtoFromTextString(param_string, param);
	if (!isok) throw std::exception("Failed to parse NetParameter");
	UpgradeNetAsNeeded("x", param);
}

void ReadNetParamsFromDataOrDie(const void* data, int length,
	NetParameter* param) {
	//CHECK(ReadProtoFromData(data, length, param))
	//	<< "Failed to parse NetParameter from data";
	bool isok = ReadProtoFromData(data, length, param);
	if (!isok) throw std::exception("Failed to parse NetParameter");
	UpgradeNetAsNeeded("x", param);
}
